...
app.post("/api/groups/join", async (req, res) => {
    ...
    if (!token || !auth0_sub)
        return res
            .status(400)
            .json({ error: "token und auth0_sub erforderlich" });
    ...
    try {
        ...
        if (!user) {
            return res
                .status(404)
                .json({
                    error: "User existiert nicht. Bitte vorher anmelden.",
                });
        }
...
app.delete("/api/groups/:groupId", async (req, res) => {
    ...
    try {
        const group = await qOne("SELECT id FROM groups WHERE id = $1", [
            groupId,
        ]);
        if (!group) {
...
app.get("/api/groups/:groupId/balances/:userId", async (req, res) => {
    ...
    const userSettlements = settlements
        ...
        .map((s) => ({
            ...
            counterparty:
                members.find(
                    (m) => m.id === (s.from === userId ? s.to : s.from)
                )?.name || "Unbekannt",
